...
const DonasiUang = () => {
  ...
  const handleTercatatChange = (id, value) => {
    setDataDonasiUang(prev =>
      prev.map(item =>
        item.id === id
          ? { ...item, tercatat: value === "tercatat" }
          : item
      )
    );
  };
    ...
    {currentData.map((item, index) => (
      <React.Fragment key={item.id}>
        <tr className="border-t">
          ...
          <td className="p-2">
            <select
              value={item.tercatat ? "tercatat" : "belum"}
              onChange={(e) => handleTercatatChange(item.id, e.target.value)}
              className="border rounded px-2 py-1"
            >
              <option value="tercatat">Tercatat</option>
              <option value="belum">Belum Tercatat</option>
            </select>
          </td>
